...
function index()
if not nixio.fs.access("/sys/devices/virtual/thermal/thermal_zone0/temp")then
return
end
entry({"admin","status","realtime","temperature1"},call("temperature1")).leaf=true
end
function temperature2(rv)
local c=nixio.fs.access("/sys/devices/virtual/thermal/thermal_zone0/temp")and
io.popen("cat /sys/devices/virtual/thermal/thermal_zone0/temp")
if c then
