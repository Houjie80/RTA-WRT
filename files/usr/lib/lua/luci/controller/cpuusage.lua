...
function index()
if not nixio.fs.access("/sys/devices/system/cpu/cpufreq/policy3/scaling_available_frequencies")then
return
end
entry({"admin","status","realtime","cpuusage1"},call("cpuusage1")).leaf=true
end
function cpuusage2(rv)
local c=nixio.fs.access("/sys/devices/system/cpu/cpufreq/policy3/scaling_available_frequencies")and
